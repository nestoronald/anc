...
<article class="item" data-permalink="<?php echo JRoute::_(ContentHelperRoute::getArticleRoute($this->item->slug, $this->item->catid), true, -1); ?>">
    ...
    <div class="content clearfix">
        <?php
            ...
            if (preg_match("<table>", $this->item->introtext)) {
                $this->item->introtext = strip_tags($this->item->introtext);
            }
            // Eliminar comentarios dentro del contenido de la lista tipo blog
            //$this->item->introtext = eregi_replace("<!--.*.-->","--",$this->item->introtext);
            // cambiar h1 por h5 en los contenidos de la lista tipo blog
            $this->item->introtext = eregi_replace("<h1[^>]*>","<h5>",$this->item->introtext);
            $this->item->introtext = eregi_replace("</h1>","<h5>",$this->item->introtext);
            echo substr($this->item->introtext, 0, 600)." ...";
